...
function tokenPairResponse(pair, fields = {}) {
  return {
    accessToken: pair.accessToken,
    refreshToken: pair.refreshToken,
    expiresIn: pair.expiresIn,
    tokenType: pair.tokenType,
    // Legacy alias for one-release compatibility
...
const verifyOtp = asyncHandler(async (req, res) => {
  ...
  if (role === "customer") {
    ...
    const pair = await issueTokenPair({
      subjectId: existing._id.toString(),
      role: "customer",
      req,
    });
    return res.json(
  ...
  const pair = await issueTokenPair({
    subjectId: principal._id.toString(),
    role: "expert",
    extra: { phone: principal.phone },
    req,
  });
...
const refresh = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(401).json({ error: err.code || "invalid_refresh_token" });
  }
});
